...
// Write to csv
int Motion::writeCSVfile(char* filename, double scale, int forceAllJointsBe3DOF)
{
    Bone* bone = pSkeleton->getRoot();
    char* pos = strstr(filename, ".amc");

    // Replace amc with csv of same name
    if (pos != nullptr) {
        std::strcpy(pos, ".csv\0"); // Copy ".csv" to the found position
    }
    std::ofstream os(filename);
    if (os.fail())
        return -1;
